...
//DATA OBJECTS
var riskTypeObject = {}
var riskTypes, riskCategories, products
var riskTypeString
...
function stepWizardInit(){
    ...
    processNavigationToStep(1)
}
function versionModeInit(){
    versionModeFillOutQuestions();
}

//NAVIGATION BUTTON HANDLERS -> ALL WILL POINT TO processNavigationToStep METHOD
function riskTypeClickAction(elem){
    //IF RISK TYPE IS ALREADY SELECTED AND CLICKED RISK TYPE IS DIFFERENT
    if(isRiskTypeSelected() && $(elem).attr('data-risktypeid') !== getSelectedRiskTypeID()){
    ...
    //REMOVE ACTIVE RISKTYPE, HIDE ANY DROPDOWNS
    $('.riskTypeDropdown').css('display', "none");
    $('a.riskOptionLink').closest('li').removeClass("active");
    $('a.riskOptionLink').closest('li').addClass("inactive");
    $(elem).closest('li').addClass("active");
    $(elem).closest('li').removeClass("inactive");

    if ($(elem).hasClass("riskOptionDropdown")) {
        $(elem).find('select').css('display', "");
    }
    else {
        //SET HEADER TO DISPLAY RISK CATEGORY AND RISK TYPE
        $('#riskCategoryHeader').html( getRiskCategoryChosen() );
        $('#riskTypeHeader').html( getRiskTypeChosen() );

        processNavigationToStep(2)
    }
...
function nextButtonClickAction(button){ 
    ...
    processNavigationToStep(gotoStepNum)
    
    if (isRiskTypeSelected()) {
        $('#loadingModal').modal('show');

        if (e.target.id == "nextButtonStep1") {
            nextButtonStep1ClickAction()
        }
...
function circleNavButtonClickAction(button){
    ...
    if (!$(button).hasClass('disabled') && !$($(button))[0].hasAttribute("disabled")) {
        if ($(button).attr('id') == "buttonCircleStep4") {
            buildReview();
        }

        processNavigationToStep(stepClicked)
    }
}

//NAVIGATION METHODS
function processNavigationToStep(stepNum){

    if(stepNum === 1){
        showStep(stepNum)
    }
    else if(stepNum === 2){
        //RISK TYPE MUST BE SELECTED TO GOTO STEP 2
        if (isRiskTypeSelected()) {
            //SHOW SAVE BUTTON
            ...
            //SET THE RISK TYPE OBJECT
            setRiskTypeObject(getSelectedRiskTypeID());
            ...
            if(riskTypeObject.riskTypeCategory === "FP"){
                if (riskTypeObject.riskTypeCode === "FP_FILM_WC" || riskTypeObject.riskTypeCode === "FP_FILM_CAST_WC") {
                    ...
                    //LOAD HTML TEMPLATES FOR RISK
                    importQuestionsForRisk(riskTypeObject.riskTypeCode)

                }
            }
            else if (getSelectedRiskCategoryID() === "EN") {
            ...
            else if (getSelectedRiskCategoryID() === "SE") {
                //alert(riskCategory)
                $('#premiumExpectedInputGroup').css('display', '');
                $('#howManyDaysIsTheEventGroup').css('display', '');
                $('#estimatedTotalAttendanceGroup').css('display', '');
            ...
            alert( $('li.active').find('.riskTypeDropdown').val() )
