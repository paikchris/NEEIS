...
var products, operations, coverages, operationCategories, conditionBasis, conditionOperators,
    questions, questionCategories, ratingBasisArray, rates, rateSheets, forms, ruleEngineObjects
...
function initializeListeners(){
    ...
    $(document.body).on('change', 'input.changeLimitValueCheckbox', function(e) {
        productOptionLimitValueCheckboxChangeAction(this)
    })
    ...
    $(document.body).on('click', '#testRateSheetButton', function(e) {
        rateSheetPage_TestWCRatesButtonAction(this)
    })


    //ADD RATE MODAL FUNCTIONS
    $(document.body).on('click', '.addRateRow', function(e) {
        checkEntireRowOnClick(this)
        e.stopPropagation()
    })
    $(document.body).on('click', '.addRateRow input.addRateCheckbox', function(e) {
        ...
        $('#' + covID + "_LogicConditionRowsContainer .logicConditionRow .conditionOutput").each(function(){
            ...
            if(productID !== null && productID != undefined){
                ...
                if(productObject.rateCode !== null && productObject.rateCode !== undefined && productObject.rateCode.trim().length > 0 && productObject.rateCode !== "NONE"){
                    ...
                    if( (rateObject !== null && rateObject !== undefined) &&
                        rateObject.rateBasis !== null && rateObject.rateBasis !== undefined && rateObject.rateBasis.trim().length > 0 && rateObject.rateBasis !== "invalid"  ){
...
function fillProductDetails(dropdown){
    ...
    if(productID ==='invalid'){
        ...
        $('#productShortDescription').val(productMap.shortDescription)
...
function buildProductObjectMap(){
    ...
    productMap.shortDescription = $('#productShortDescription').val()
...
function updateRateValueDisplay(){
    ...
    if(rateMap.rateBasis === 'BRACKET'){
        ...
        var ratingBasisObject = getRatingBasisObjectByID(rateBasisID)
        var questionID = rateMap.tieredQuestionID
...
function fillProductLimits(productID){
    ...
    if(productLimitArray !== undefined && productLimitArray!== null && productLimitArray.length > 0){
        for(var i=0; i<productLimitArray.length; i++){
            var limitAmount, limitDescription, optionFlag
            var additionalPremium = ""
            var applyMinPremium = ""
            var changeLimitValueFlag = ""
            ...
            if(productObject.limitArray){
                ...
                if(thisLimitMap.limitProductOption !== 'undefined'){
                    optionFlag = thisLimitMap.limitProductOption
                }
                if(thisLimitMap.productOptionAdditionalPremium){
                    additionalPremium = thisLimitMap.productOptionAdditionalPremium
                }
                if(thisLimitMap.applyMinPremium){
                    applyMinPremium = thisLimitMap.applyMinPremium
                }
                if(thisLimitMap.changeLimitValueFlag){
                    changeLimitValueFlag = thisLimitMap.changeLimitValueFlag
                }
                if(thisLimitMap.limitValueChange){
                    limitValueChange = thisLimitMap.limitValueChange
                }
                if(thisLimitMap.limitDescriptionChange){
                    limitDescriptionChange = thisLimitMap.limitDescriptionChange
                }
            }
            else{
            ...
            limitHTML = limitHTML + getNewLimitRowHTML(limitAmount, limitDescription, optionFlag, additionalPremium, applyMinPremium, changeLimitValueFlag, limitValueChange, limitDescriptionChange)
        }
    }
    else{
...
function getNewLimitRowHTML(limitAmount, limitDescription, optionFlag, additionalPremium, applyMinPremium, changeLimitValueFlag, limitValueChange, limitDescriptionChange){
    var limitRowHTML = "<div class='row productPage_limitRow limDeductRow'> " +
        ...
        "   <div class='col-xs-1'> " +
        "       <label class='checkBoxLabel'>" +
        "           <input type='checkbox' class='limitLineProductOption onChangeSaveProduct' data-limitdescription='" + limitDescription + "' " +
        ( optionFlag ? " checked='true' " : "" ) +
        "/>" +
        "       </label>" +
        "       <label class='checkBoxLabel'>" +
        "           <input type='checkbox' class='applyMinPremiumCheckbox onChangeSaveProduct' data-limitdescription='" + limitDescription + "' " +
        ( applyMinPremium ? " checked='true' " : "" ) +
        "/>" +
        "       </label>" +
        "       <label class='checkBoxLabel'>" +
        "           <input type='checkbox' class='changeLimitValueCheckbox' data-limitdescription='" + limitDescription + "' " +
        ( changeLimitValueFlag ? " checked='true' " : "" ) +
        "/>" +
        "       </label>" +
        "   </div> " +
        "   <div class='col-xs-2'> " +
        "       <div class='form-group'> " +
        "           <input class='form-control input-xs productOptionAdditionalPremium onChangeSaveProduct maskMoney' type='text' " +
        ( additionalPremium ? " value='" + additionalPremium + "' " : "" ) +
        "/>" +
        "       </div> " +
        "   </div> " +
        "   <div class='col-xs-2'>" +
        ...
        "       </button>" +
        "   </div>" +
        ( limitValueChange || limitDescriptionChange ? getLimitValueChangeOptionsHTML(limitValueChange, limitDescriptionChange) : "" ) +
        "</div>"
...
function buildLimitArray(){
    ...
    $(container).find('.productPage_limitRow').each(function(){
        var limitAmount = $(this).find('.limitAmount').val()
        var limitDescription = $(this).find('.limitDescription').val()
        var limitProductOption = $(this).find('.limitLineProductOption').is(':checked')
        var productOptionAdditionalPremium
        var applyMinPremium
        var changeLimitValueFlag
        var limitValueChange
        var limitDescriptionChange

        if(limitProductOption){
            productOptionAdditionalPremium = $(this).find('.productOptionAdditionalPremium').val()
            applyMinPremium = $(this).find('.applyMinPremiumCheckbox').is(':checked')
            changeLimitValueFlag = $(this).find('.changeLimitValueCheckbox').is(':checked')

            if(changeLimitValueFlag){
                limitValueChange = $(this).find('.productOptionLimitValue').val().trim()
                limitDescriptionChange = $(this).find('.productOptionLimitDescription').val()
            }
        ...
        var tempLimitObject = {
            limitAmount: limitAmount,
            limitDescription: limitDescription,
            limitProductOption: limitProductOption,
            productOptionAdditionalPremium: productOptionAdditionalPremium,
            applyMinPremium: applyMinPremium,
            changeLimitValueFlag: changeLimitValueFlag,
            limitValueChange: limitValueChange,
            limitDescriptionChange: limitDescriptionChange
        }
...
function productOptionLimitValueCheckboxChangeAction(checkbox){
    var limitRowElement = $(checkbox).closest('.limDeductRow')

    if( $(checkbox).is(':checked') ){
        $(limitRowElement).append( $(getLimitValueChangeOptionsHTML('', '')) )
    }
    else{
        $(limitRowElement).find('.productOptionLimitOptionsContainer').remove()
    }



}
function getLimitValueChangeOptionsHTML(limitValueChange, limitDescriptionChange){
    var htmlString = "" +
        "<div class='col-xs-12 productOptionLimitOptionsContainer'>" +
        "   <div class='col-xs-2'>" +
        "   </div>" +
        "   <div class='col-xs-2'>" +
        "       <div class='form-group'> " +
        "           <input class='form-control input-xs productOptionLimitValue onChangeSaveProduct' type='text' " +
        "           placeholder='Limit Value' value='" + limitValueChange + "' />" +
        "       </div> " +
        "   </div>" +
        "   <div class='col-xs-4'>" +
        "       <div class='form-group'> " +
        "           <input class='form-control input-xs productOptionLimitDescription onChangeSaveProduct' type='text' " +
        "           placeholder='Limit Desc' value='" + limitDescriptionChange + "' />" +
        "       </div> " +
        "   </div>" +
        "</div>"
...
/////////////////////RATE SHEETS PAGE FUNCTIONS///////////////////
function rateSheetRowHeaderHTML(){
    var htmlString = "" +
        "<div class='row'>" +
        "   <div class='col-xs-1'>" +
        "       <label>Type</label>" +
        ...
        "   </div>" +
        "   <div class='col-xs-1'>" +
        "       <label>Flat Prem</label>" +
        "   </div>" +
        "   <div class='col-xs-2'>" +
        "       <label>Rate Method</label>" +
        "   </div> " +
        "   <div class='col-xs-2'> " +
        "       <label>Subj. To Mod.</label> " +
        "   </div> " +
        "</div>"
}
function rateSheetRowHTML(rateObject){
    var htmlString = "" +
        ...
        "   </div>" +
        "   <div class='col-xs-1'>" +
        "       <span>" + (rateObject.rate ? rateObject.rate : "") + "</span>" +
        "   </div>" +
        "   <div class='col-xs-1'>" +
        "       <span>" + (rateObject.premium ? rateObject.premium : "") + "</span>" +
        "   </div>" +
        "   <div class='col-xs-1'>" +
        "       <span>" + (rateObject.nonMod ? rateObject.nonMod : "") + "</span>" +
        "   </div>" +
        "   <div class='col-xs-1'>" +
        "       <span>" + (rateObject.flat ? rateObject.flat : "") + "</span>" +
        "   </div>" +
        "   <div class='col-xs-1'>" +
        ...
        "       </button>" +
        "       <button type='button' class='btn btn-xs btn-info rateSheetRowMoveDownButton' style='font-size:9px'>" +
...
function resetRateSheetRows(){
    $('.rateSheetRow').remove()
}
function rateSheetsPage_RateDropdownAction(dropdown){
    if(getSelectedRateSheetID() !== 'invalid'){
        ...
        //FILL RATE SHEET ID
        $('#rateSheetIDInput').val(rateSheetID)
        ...
        showRateSheetDetails()
        showRateSheetContainer()
    }
    else{
        resetRateSheetPage()
        hideRateSheetDetails()
...
function buildWCRateSheetRowsHTML(rateSheetArray, state){
    ...
    if(rateSheetArray[state] && rateSheetArray[state].length > 0){
        var stateRateCodeArray = rateSheetArray[state]

        //LOOP THROUGH THE STATES RATE CODES, INSERT INTO RIGHT CONTAINERS
        for(var i=0;i<stateRateCodeArray.length;i++){
            var rateCode = stateRateCodeArray[i]
        ...
        //IF RATE ROWS EXIST, SHOW THE TEST RATE SHEET CONTAINER
        if( $('.rateSheetRow').length > 0 ){
            buildRateSheetTestInputs()
            showRateSheetTestContainer()
        }
        else{
            clearRateSheetTestInputs()
            hideRateSheetTestContainer()
        }

    }
    else{
        //IF NO RATE CODES EXIST FOR STATE SELECTED
        resetRateSheetRows()
        hideRateSheetTestContainer()
    }
}
...
function hideRateSheetDetails(){
    $('#rateSheetDetailsContainer').css('display', 'none')
}
function showRateSheetDetails(){
    $('#rateSheetDetailsContainer').css('display', '')
}
function hideRateSheetContainer(){
    $('#rateSheetContainer').css('display', 'none')
}
...
function checkEntireRowOnClick(elementClicked){
    if( !$(elementClicked).is(':checkbox')){
        var rowElement = $(elementClicked).closest('.addRateRow')
        $(rowElement).find('.addRateCheckbox').trigger('click')
    }
}
...
function formatRateSheetRows(){
    $('.rateSheetRow').each(function(index){
        ...
        //NUMBER EACH ROW
        $(thisRow).find('.rateSheetLineNumber').html(index+1)


        //FIX ROW COLORS
        if( $(thisRow).index() % 2 === 0 ){
            $(thisRow).css('background-color', 'rgba(230, 230, 230, 0.31)')
        }
        else{
...
function moveRateSheetRowUp(upButton){
    var container = $(upButton).closest('.rowContainer')
    var thisRow = $(upButton).closest('.rateSheetRow')
    ...
    if(thisRowIndex === 0){
        ...
        $(container).children('.rateSheetRow').eq(thisRowIndex-1).before($(thisRow))
    }

    formatRateSheetRows()
}
function moveRateSheetRowDown(downButton){
    var container = $(downButton).closest('.rowContainer')
    var thisRow = $(downButton).closest('.rateSheetRow')
    var thisRowIndex = $(thisRow).index()
    var lastIndex = $(container).children('.rateSheetRow').length

    if(thisRowIndex === lastIndex){

    }
    else{
        $(container).children('.rateSheetRow').eq(thisRowIndex+1).after($(thisRow))

    }

    formatRateSheetRows()
}
function deleteRateSheetRow(deleteButton){
...
function buildWCRateSheetJSON(){
    ...
    if(currentRateSheetObject){
        ...
        $('#wcRateSheetRowsContainer .rateSheetRow').each(function(){
            var rateCode = $(this).attr('data-rateid')

            if(rateCode.length > 0){
                stateRateCodeArray.push(rateCode)
            }
        });
...
function hideRateSheetTestContainer(){
    $('#wcRateSheetTestContainer').css('display', 'none')
}
function showRateSheetTestContainer(){
    $('#wcRateSheetTestContainer').css('display', '')
}
function hideRateSheetTestResultsContainer(){
    $('#wcRateSheetTestResultsContainer').css('display', 'none')

}
function showRateSheetTestResultsContainer(){
    $('#wcRateSheetTestResultsContainer').css('display', '')
}

function testRateInputHTML(rateCode){
    var htmlString = "" +
        "<div class='col-xs-2'>" +
        "   <div class='form-group'>" +
        "       <h6>" + rateCode + "</h6>" +
        "       <input class='form-control rateSheetTestInput maskMoney' " +
        "           data-ratecode='" + rateCode + "' " +
        "           data-precision='0' data-prefix='$' type='text'>" +
        "   </div>" +
        "</div>"

    return htmlString
}
function clearRateSheetTestInputs(){
    $('#rateSheetTestInputsContainer').html('')
}
function clearRateSheetTestResultRows(){
    $('#wcRateSheetRowsContainer_Test .rateSheetTestRow').remove()
}
function rateSheetPage_TestWCRatesButtonAction(){
    buildRateSheetTestDisplay()
    showRateSheetTestResultsContainer()
}
function buildRateSheetTestInputs(){
    clearRateSheetTestInputs()

    //ANY RATE CODE THAT IS PROCESSED BY 2RATE NEEDS A INPUT
    $('#2RATE_rateSheetRowsContainer .rateSheetRow').each(function(){
        var rateCode = $(this).attr('data-rateid')

        $('#rateSheetTestInputsContainer').append(testRateInputHTML(rateCode))
    })

    $('.rateSheetTestInput').maskMoney()
    $('.rateSheetTestInput').maskMoney('mask')
}
function buildRateSheetTestDisplay(){
    //CLEAR RATE SHEET TEST RESULTS
    clearRateSheetTestResultRows()

    //GET ARRAY OF RATE CODES IN RATE SHEET
    var rateCodeArray = []
    $('.rateSheetRow').each(function(){
        var rateCode = $(this).attr('data-rateid')

        rateCodeArray.push(rateCode)
    })
    ...
    for(var i=0; i<rateCodeArray.length; i++){
        ...
        if(howProcessed === "1STD"){
            ...
            testResultRowObject.modPrem = Math.round( ( parseFloat(testResultRowObject.basisValue) * parseFloat(testResultRowObject.rate) ) / 100 )

            $('#' + howProcessed + '_rateSheetRowsContainer_Test').append(rateSheetTestResultRow(testResultRowObject))
        }
            ...
            $('#2RATE_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
                if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
                    payrollPremiumTotal = payrollPremiumTotal + getFloatValueOfMoney($(this).html().trim())
                }
            })
            ...
            $('#' + howProcessed + '_rateSheetRowsContainer_Test').append(rateSheetTestResultRow(elMPRowObject))
        }
        else if(howProcessed === '5MP'){
            //ADD ALL PREMIUM ROWS
            var ratedPremiumTotal = 0
            $('#2RATE_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
                if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
                    ratedPremiumTotal = ratedPremiumTotal + getFloatValueOfMoney($(this).html().trim())
                }
            })
            $('#3PREM_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
                if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
                    ratedPremiumTotal = ratedPremiumTotal + getFloatValueOfMoney($(this).html().trim())
                }
            })
            $('#4EL_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
                if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
                    ratedPremiumTotal = ratedPremiumTotal + getFloatValueOfMoney($(this).html().trim())
                }
            })
            ...
            if( ( parseFloat(minPrem) - parseFloat(ratedPremiumTotal) ) > 0 ){ //NOT MET
                testResultRowObject.modPrem = parseFloat(minPrem) - parseFloat(ratedPremiumTotal)
            }
            else{ //MET
            ...
            testResultRowObject.basisValue = ratedPremiumTotal
            ...
            var employersLiabilityPrem = 0
            $('#4EL_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
                if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
                    employersLiabilityPrem = employersLiabilityPrem + getFloatValueOfMoney($(this).html().trim())
                }
            })

            var premDiscountBasisValue = 0
            if( (parseFloat(payrollPremiumTotal) + parseFloat(employersLiabilityPrem)) > parseFloat(rateCodeObject.premium) ){//IF PREMIUM IS HIGH ENOUGH FOR DISCOUNT
                premDiscountBasisValue = (parseFloat(payrollPremiumTotal) + parseFloat(employersLiabilityPrem)) - parseFloat(rateCodeObject.premium)
            }
            ...
            var payrollTotal = 0
            $('#2RATE_rateSheetRowsContainer_Test .rateSheetTestRow span.basisValueSpan').each(function(){
                if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
                    payrollTotal = payrollTotal + getFloatValueOfMoney($(this).html().trim())
                }
            })

            var rate = rateCodeObject.rate

            var premium = Math.round( ( parseFloat(payrollTotal) * parseFloat(rate) ) / 100 )
            if(rateCodeObject.nonMod === '1' ){
            ...
            testResultRowObject.basisValue = payrollTotal
            ...
            $('#2RATE_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
                if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
                    totalPremium = totalPremium + getFloatValueOfMoney($(this).html().trim())
                }
            })
            $('#3PREM_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
                if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
            ...
            $('#5MP_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
                if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
                    totalPremium = totalPremium + getFloatValueOfMoney($(this).html().trim())
                }
            })
            $('#6PDISC_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
                if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
                    totalPremium = totalPremium + getFloatValueOfMoney($(this).html().trim())
                }
            })
            $('#FEE_PR_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
                if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
                    totalPremium = totalPremium + getFloatValueOfMoney($(this).html().trim())
                }
            })

            testResultRowObject.basisValue = totalPremium
            testResultRowObject.rate = rateCodeObject.rate

            var premium = Math.round( ( parseFloat(totalPremium) * parseFloat(rateCodeObject.rate) ) / 100 )
            if(rateCodeObject.nonMod === '1' ){
                testResultRowObject.otherPrem = premium
            }
            else{
            ...
            var basisValue = parseFloat(payrollPremiumTotal) + parseFloat(employersLiabilityPrem)
    ...
    //BUILD TOTAL PREMIUM
    var premiumFinalTotal = 0
    $('#2RATE_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
        if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
            premiumFinalTotal = premiumFinalTotal + getFloatValueOfMoney($(this).html().trim())
        }
    })
    $('#3PREM_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
        if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
    ...
    $('#FEE_EL_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
        if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
            premiumFinalTotal = premiumFinalTotal + getFloatValueOfMoney($(this).html().trim())
        }
    })
    ...
    //TOTAL FEES
    var feeFinalTotal = 0
    $('#FEE_RATE_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
        if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
            feeFinalTotal = feeFinalTotal + getFloatValueOfMoney($(this).html().trim())
        }
    })
    $('#FEE_ME_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
        if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
            feeFinalTotal = feeFinalTotal + getFloatValueOfMoney($(this).html().trim())
        }
    })
    $('#FEE_MEL_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
        if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
            feeFinalTotal = feeFinalTotal + getFloatValueOfMoney($(this).html().trim())
        }
    })
    $('#FEE_WV_rateSheetRowsContainer_Test .rateSheetTestRow span.premiumSpan').each(function(){
        if( $(this).html().trim().length > 0 && $(this).html().trim() !== '-'){
            feeFinalTotal = feeFinalTotal + getFloatValueOfMoney($(this).html().trim())
        }
    })
    ...
    //INSERT PREMIUM AND FEE TOTAL
    $('#rateSheetTestResultTotalPremium').html(formatMoney(premiumFinalTotal))
    $('#rateSheetTestResultTotalFee').html(formatMoney(feeFinalTotal))
    $('#rateSheetTestResultTotalPremiumAndFee').html(formatMoney(parseFloat(premiumFinalTotal) + parseFloat(feeFinalTotal)))


}

function rateSheetTestResultRow(resultObject){
    var htmlString = "" +
        "<div class='row rateSheetTestRow'>" +
        "   <div class='col-xs-1'>" +
        "       <span>" + (resultObject.code !== undefined ? resultObject.code : '' ) +
        "       </span>" +
        "   </div>" +
        "   <div class='col-xs-3'>" +
        "       <span>" + (resultObject.description !== undefined ? resultObject.description : '' ) +
        "       </span>" +
        "   </div>" +
        "   <div class='col-xs-2'>" +
        "       <span class='basisValueSpan'>" + (resultObject.basisValue !== undefined ? formatMoney(resultObject.basisValue) : '' ) +
        "       </span>" +
        "   </div>" +
        "   <div class='col-xs-2'>" +
        "       <span>" + (resultObject.rate !== undefined ? resultObject.rate : '' ) +
        "       </span>" +
        "   </div>" +
        "   <div class='col-xs-2' style='background: #add8e63b;'>" +
        "       <span class='premiumSpan modPremium'>" + (resultObject.modPrem !== undefined ? formatMoney(resultObject.modPrem) : '-' ) +
        "       </span>" +
        "   </div>" +
        "   <div class='col-xs-2' style='background: #add8e63b;'>" +
        "       <span class='premiumSpan otherPremium'>" + (resultObject.otherPrem !== undefined ? formatMoney(resultObject.otherPrem) : '-' ) +
        "       </span>" +
        "   </div>" +
        "</div>"
...
function fillRateDetails(rateID){
    ...
    $('#rateCodeInput').val(rateObject.rateCode)
    $('#rateDescriptionInput').val(rateObject.description)
    $('#rateTypeDropdown').val(rateObject.rateType)
    $('#rateMethodDropdown').val(rateObject.rateMethod)
    ...
    if(rateObject.rateBasis === 'LIMIT'){
        ...
        $('#ratesPage_BracketBasisQuestionID').val(rateObject.tieredQuestionID)
    }
...
function showLimitRateValueContainer(){
    ...
    $('#ratePage_WCRateValueContainer').css('display', 'none')
...
function showWCRateContainer(){
    ...
    //SHOW LIMIT RATE VALUE CONTAINER
    $('#ratePage_WCRateValueContainer').css('display', '')
}
...
function getCurrentSelectedRateSheetID(){
    return $('#rateSheetPage_RateSheetsDropdown').val()
}
...
function saveOperation(){
    ...
    $.ajax({
        ...
        data: {
            ...
            rulesEngineMap: JSON.stringify(buildRulesEngineMapForOperation())
        }
    })
        .done(function(msg) {
            if(msg === "Success"){
...
function saveRate(saveRatebutton){
    ...
    dataMap.rateCode = $('#rateCodeInput').val()
    dataMap.description = $('#rateDescriptionInput').val()
    dataMap.rateType = $('#rateTypeDropdown').val()
    dataMap.rateMethod = $('#rateMethodDropdown').val()
    ...
    if(dataMap.rateBasis === "LIMIT"){
        ...
        dataMap.tieredQuestionID = $('#ratesPage_BracketBasisQuestionID').val().trim()
    }
...
function validateRatePage(){
    ...
    if(ratingBasisID){
        if(ratingBasisID === 'LIMIT'){
            ...
            if($('#ratesPage_BracketBasisQuestionID').val().trim().length === 0){
                valid = false
            }
        }
...
function saveRateSheet(){
    ...
    $.ajax({
        method: "POST",
        url: "/Admin/saveRateSheetChanges",
        data: dataMap
    })
        .done(function(msg) {
            if(msg === "Success"){
...
function createNewRateSheet(){
    //CHECK IF ALL REQUIRED FIELDS ARE FILLED
    var rateSheetID = $('#newRateSheet_rateSheetID').val().trim()
    var rateSheetName = $('#newRateSheet_rateSheetID').val().trim()

    if(rateSheetID.length === 0 || rateSheetName.length === 0 ){
        alert("Complete All Fields")
    }
    else{
        $.ajax({
            method: "POST",
            url: "/Admin/createRateSheetRecord",
            data: {
...
function refreshRateSheets(){
    $.ajax({
        method: "POST",
        url: "/Admin/refreshRateSheets",
        data: {
        }
    })
        .done(function(msg) {
            if(msg === "Error"){
                ...
                rateSheets = jsonStringToObject(msg)

                //UPDATE DROPDOWNS
                for(var i=0;i<rateSheets.length;i++){
                    if( $('#rateSheetPage_RateSheetsDropdown').find("option[value='" + rateSheets[i].rateSheetID + "']").length === 0 ){
                        var htmlElement = $(
                            "<option value='" + rateSheets[i].rateSheetID + "'>" +
                            "" + rateSheets[i].rateSheetID + " - " + rateSheets[i].description +
                            "</option>"
                        )

                        $('#rateSheetPage_RateSheetsDropdown').append( $(htmlElement) )
                    }
                }
