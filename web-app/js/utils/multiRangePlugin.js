...
function initMultiRange(){
    ...
    $(document).on('change', '#limits_TestInput', function (e) {
        $('.covLimitLimitInput').val( $(this).val() )
    });
...
function recreateRateRangesFromMap(rateInfoMap){
    if(rateInfoMap.basisName === "gpc"){
        ...
        for(var i = 0; i<rateRangeRowsArray.length; i++){
            ...
            //IF ADDITIONAL CONDITIONS EXIST FOR THIS COVERAGE LIMIT
        ...
        //LOOP THROUGH ALL COVERAGE LIMIT RATE INFOS
        $('#limits_TotalMinPremium').val(rateInfoMap.totalMinPrem)
        for (var property in rateInfoMap) {
            if (property.indexOf('covLimitRateInfo_') > -1 && rateInfoMap.hasOwnProperty(property)) {
                var tempMap = rateInfoMap[property]
                var covLimitName = tempMap.covLimitName
                var covLimitRateRangeRowsArray = tempMap.rateInfo
                var covLimitID = property.split('_')[1]

                //console.log(covLimitName)
                //console.log(tempMap)

                var covLimitRangeContainer = $('#rangeContainerCovLimit_' + covLimitID)

                //MAKE SURE RANGE CONTAINER IS EMPTY, TO RECREATE FROM SCRATCH
                covLimitRangeContainer.empty()
                //SETUP HEADER ROW
                covLimitRangeContainer.html(headerRowHTML_Limits())



                //RECREATE MAIN RANGE ROWS
                for(var i=0; i<covLimitRateRangeRowsArray.length; i++){
                    //console.log(covLimitRateRangeRowsArray[i])
                    var thisRowMap = covLimitRateRangeRowsArray[i]
                    ...
                    covLimitRangeContainer.append(rangeRow)

                }
                addRemoveRangeButtons(covLimitRangeContainer)
    ...
    if($(element).val() === 'termLength' || $(element).val() === 'Term Length'){
        if( $(rangeRowContainer).children('.additionalConditionDiv').length === 0 ){
            ...
            if( $(element).closest('.multiRangeContainer').hasClass('limits') ){
                var covLimitID = $(element).closest('.multiRangeContainer.limits').attr('data-covLimitid')
                var covLimitName = $(element).closest('.multiRangeContainer.limits').attr('data-covLimitname')
                newMultiRangeContainer = $(
                    "<div class='row additionalConditionDiv multiRangeContainer days' id='additionalRangeContainerCovLimit_" + covLimitID + "' " +
                    "       data-covLimitid='" + covLimitID + "' data-covLimitname='" + covLimitName + "' style='padding-top:8px; padding-bottom:16px;'>" +
                    "</div> "
                );
            }
            else{
        ...
        if( $(rangeRowContainer).children('.additionalConditionDiv').length === 0 ){
            ...
            if( $(element).closest('.multiRangeContainer').hasClass('limits') ){
                ...
                var covLimitName = $(element).closest('.multiRangeContainer.limits').attr('data-covLimitname')
                newMultiRangeContainer = $(
                    "<div class='row additionalConditionDiv multiRangeContainer money' id='additionalRangeContainerCovLimit_" + covLimitID + "' " +
                    "       data-covLimitid='" + covLimitID + "' data-covLimitname='" + covLimitName + "' style='padding-top:8px; padding-bottom:16px;'>" +
                    "</div> "
                );
            }
...
function buildRateInfoMap(rangeContainer, basis){
    ...
    if(basis === "gpc" || basis === "termLength" || basis === "budgetAgg"){
        ...
        $(multiRangeContainer).children('.rangeRow').each(function(){
            ...
            //IF THIS IS A COVERAGE LIMIT LIMIT ADDITIONAL CONDITION
            if( typeof $(rangeContainer).attr('data-covLimitid') !== typeof undefined && $(rangeContainer).attr('data-covLimitid') !== false ){
                var covLimitID = $(rangeContainer).attr('data-covLimitid')
                var covLimitName = $(rangeContainer).attr('data-covLimitname')

                thisRangeRateInfo.covLimitID = covLimitID
            }
        ...
        $('#limitRatingCovLimitRangeContainer > div > div.col-xs-12 > .multiRangeContainer').each(function(c){
            var thisRateInfoArray = []
            var multiRangeContainer = $(this)
            var covLimitName = $(this).closest('.covLimitRateInfoContainer').find('.covLimitStringDisplay').html().trim()
            var covLimitID = $(this).attr('data-covLimitid')

            var limit_RateInfoMap = {}
            limit_RateInfoMap.covLimitName = covLimitName
            ...
            rateInfoMap['covLimitRateInfo_' + covLimitID] = limit_RateInfoMap
        })
    }
...
function runRatePreview(){
    ...
    if(primaryRatingBasis === "limits"){
        $('.covLimitLimitInput').each(function(){
            var tempMap = {}
            tempMap.covLimitID = $(this).attr('data-covLimitID')
            tempMap.limitValue = $(this).val()
            tempMap.limitName = $(this).attr('data-covLimitName')
            tempMap.gpc = policyInfoMap.gpc
            tempMap.termLength = policyInfoMap.termLength

            policyInfoMap['covLimit_' + $(this).attr('data-covLimitID')] = tempMap
        })
    }
